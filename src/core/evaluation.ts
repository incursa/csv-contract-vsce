...
export function resolveEvaluation(contract: CsvContract, evaluatedAt: string): CsvContract {
  ...
  return { ...contract, rules: contract.rules?.map(rule => ({ ...rule, when: rule.when ? resolve(rule.when) : undefined, expect: resolve(rule.expect) })),
    groupRules: contract.groupRules?.map(rule => ({ ...rule, when: rule.when ? resolve(rule.when) : undefined })) };
}

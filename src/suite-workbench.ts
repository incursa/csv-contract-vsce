...
export function renderSuiteWorkbench(state: SuiteWorkbenchState, nonce: string): string {
  ...
  <div class="toolbar"><button class="primary" data-action="run" ${!suite || state.running ? "disabled" : ""}>${state.running ? "Running suite…" : "Run suite"}</button><button data-action="preflight">Test connections / preflight</button><button data-action="bulk-connection">Bulk edit connections</button><button data-action="watch-inputs">${state.watchInputs ? "Stop watching CSV input" : "Watch CSV input changes"}</button><button data-action="history">Run history</button><button data-action="live">${state.live ? "Pause live tests" : "Enable live tests"}</button><button data-action="cancel">Cancel</button><button data-action="selected">Run selected members</button><button data-action="failed">Rerun failed members</button><button data-action="sql" ${!suite || state.running ? "disabled" : ""}>Generate SQL</button><button data-action="connection" ${!suite || state.running ? "disabled" : ""}>Edit default connection</button><button data-action="credentials" ${state.running ? "disabled" : ""}>Configure profile credentials</button><button data-action="export" ${!state.runs || state.running ? "disabled" : ""}>Export errors / results</button><button data-action="yaml">Edit suite YAML</button></div>
  ...
  <div class="section-heading"><h2>Suite members</h2><input id="filter" type="search" aria-label="Filter members" placeholder="Filter members or tables"></div><section aria-label="Suite members">${cards}</section>${state.runs?.some(r => r.member.startsWith("cross:")) ? `<section><h2>Cross-table checks</h2>${renderResults(state.runs.filter(r => r.member.startsWith("cross:")), "", state.stale)}</section>` : ""}<p id="no-match" class="muted" hidden>No members match your filter.</p></main>
  <script nonce="${nonce}">
